...
from .utils import PLUM_ANSWER_LIST, SHORT_QUESTION_LIST
...
def init_partonomy(base_image_dir):
    class_hierarchy_path = os.path.join(base_image_dir, "partonomy", "graph.yaml")
    if os.path.exists(class_hierarchy_path):
        with open(class_hierarchy_path, "r") as fp:
            class_hierarchy = yaml.safe_load(fp)
    else:
        raise FileNotFoundError(f"class_hierarchy_path has no file: {class_hierarchy_path} ")

    class_hierarchy_dict = {}
    partonomy_classes, partonomy_images, partonomy_labels = [], [], []

    # load the component_graph and parse them into superordinate, coarse and parts (all in a single dict - class_hierarchy_dict)
    for super_coarse, parts_list in class_hierarchy['component_graph'].items():
        superordinate = super_coarse.strip().split('--')[0]  # e.g., airplanes--attack
        coarse = super_coarse.strip().split('--')[-1]
        if superordinate not in class_hierarchy_dict:
            class_hierarchy_dict[superordinate] = defaultdict(list)
        class_hierarchy_dict[superordinate][coarse].extend([part.split(':')[-1].strip() for part in parts_list])
        print(">> class_hierarchy_dict: ", class_hierarchy_dict)

        # each concept and part will be an atomic concept (e.g., airplanes--attack is a class and airplanes--attack--part:bomb racks is a class on its own)
        partonomy_classes.append(super_coarse)
        partonomy_classes.extend(parts_list)
    
    partonomy_classes = list(set(partonomy_classes))  # remove any duplicate class
    for cls_lbl in partonomy_classes:
        partonomy_images.extend(glob.glob(os.path.join(base_image_dir, "partonomy", "images", cls_lbl, "*.jpg")))
        partonomy_labels.extend(glob.glob(os.path.join(base_image_dir, "partonomy", "annotations", cls_lbl, "*.json")))

    return partonomy_classes, partonomy_images, partonomy_labels
...
class SemSegDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        per_token_label_dict = [{k: v} for k, v in zip(sampled_classes, masks)]
        if len(conversations) > len(per_token_label_dict):
            per_token_label_dict.extend([{} for _ in range(len(conversations) - len(per_token_label_dict))])
