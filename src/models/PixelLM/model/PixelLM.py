        ...
        **kwargs,
        ...
        if inference:
            ...
            for i in range(n_batch):
                ...
                ce_losses.append(output_i.loss)
                # torch.cuda.empty_cache()
        ...
        for i in range(len(seg_token_offset) - 1):
            ...
            if len(batch_pred_embeddings) / (seg_token_num*feat_scale_num) != 0:
                batch_pred_embeddings = batch_pred_embeddings.repeat_interleave(feat_scale_num, dim=0).view(-1, feat_scale_num, seg_token_num, batch_pred_embeddings.shape[-1])
            else:
