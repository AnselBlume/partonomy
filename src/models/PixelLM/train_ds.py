...
from models.lisa.utils.explanatory_seg_datasets_adapter import ExplanatorySegDatasetsAdapter
...
def parse_args(args):
    ...
    # Partonomy args
    parser.add_argument("--partonomy_train_dataset_path", default="dataset/partonomy_descriptors/{dataset_split}/{dataset_split}_qa_pairs_train.json", type=str)
    parser.add_argument("--partonomy_val_dataset_path", default="dataset/partonomy_descriptors/{dataset_split}/{dataset_split}_qa_pairs_val.json", type=str)
    ...
    # Evaluators
    parser.add_argument('--metrics_macro_giou_evaluator_config', default=IoUEvaluatorConfig(reduction=Reduction.MACRO), type=IoUEvaluatorConfig)
    parser.add_argument('--metrics_micro_giou_evaluator_config', default=IoUEvaluatorConfig(reduction=Reduction.MICRO), type=IoUEvaluatorConfig)
    parser.add_argument('--metrics_part_text_evaluator_config', default=PartTextEvaluatorConfig(), type=PartTextEvaluatorConfig)
...
def main(args):
    ...
    if args.local_rank == 0:
        ...
        run_name = f"{args.exp_name}-ft-{'_'.join(args.partonomy_dataset_split.split('|'))}"
        run_ckpt_path = f"{args.exp_name}-ft-{'_'.join(args.partonomy_dataset_split.split('|'))}_ckpt"
    ...
    if args.explanatory_seg_data and 'pixellm' in args.version.lower():
        print(">> (train_ds) loading ExplanatorySegDataset...")
        ...
        train_paths = [args.partonomy_train_dataset_path.format(dataset_split=dataset_split) for dataset_split in dataset_splits]
        ...
        explanatory_seg_dataset_adapter = ExplanatorySegDatasetsAdapter(explanatory_seg_datasets, only_return_correct_conversation=True, indexing_strategy='concatenate')
    ...
    if args.no_eval == False:
        ...
        if args.explanatory_seg_data and 'pixellm' in args.version.lower():
            ...
            val_dataset = ExplanatorySegDatasetsAdapter(explanatory_seg_val_datasets, only_return_correct_conversation=True, indexing_strategy='concatenate', inference=True)
            print(f">> ExplanatorySegDataset ({args.partonomy_question_type}): Validating with {len(val_dataset)} examples.")
        elif len(args.val_dataset.split('||')) == 1:
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.no_eval == False:
            if args.explanatory_seg_data and 'pixellm' in args.version.lower():
                giou, ciou, macro_gt_mask_metrics, micro_gt_mask_metrics = validate_partonomy(val_loader, model_engine, epoch, writer, args)
        ...
        if args.no_eval or is_best:
            save_dir = os.path.join(args.log_dir, run_ckpt_path)
            if args.local_rank == 0:
                torch.save(
                    {"epoch": epoch},
                    os.path.join(
        ...
        save_dir = os.path.join(args.log_dir, "pixellm-13b-ft-partimagenet-paco_lvis-pascal_part_ckpt")
        if args.local_rank == 0:
            if os.path.exists(save_dir):
                shutil.rmtree(save_dir)
        torch.distributed.barrier()
...
def validate_partonomy(
    ...
    macro_iou_evaluator = IoUEvaluator(args.metrics_macro_giou_evaluator_config)
    micro_iou_evaluator = IoUEvaluator(args.metrics_micro_giou_evaluator_config)
    ...
    for batch_idx, input_dict in enumerate(tqdm.tqdm(val_loader, total=n_batches, desc="Validating Partonomy with PLUM")):
        ...
        macro_gt_mask_metrics = macro_iou_evaluator.update(output_list.cpu().numpy(), masks_list.cpu().numpy())
        micro_gt_mask_metrics = micro_iou_evaluator.update(output_list.cpu().numpy(), masks_list.cpu().numpy())
    ...
    return giou, ciou, macro_gt_mask_metrics, micro_gt_mask_metrics
