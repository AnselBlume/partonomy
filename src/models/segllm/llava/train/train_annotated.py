# NOTE: This file is an annotated companion to ``train.py``. It preserves the
# executable logic but adds commentary about the data preparation pipeline and
# model wiring that drive SegLLM training. The annotations are geared toward
# adapting the system so that a single response can emit multiple segmentation
# masks with intra-response feature passing.
#
...
def rank0_print(*args):
    """Print only on process rank 0.

    Returns:
        None: Convenience wrapper for distributed logging.
    """
    if local_rank == 0:
        print(*args)
...
def maybe_zero_3(param, ignore_status=False, name=None):
    """Materialize DeepSpeed ZeRO partitioned parameters on CPU.

    Returns:
        torch.Tensor: A detached CPU copy of ``param``.
    """
...
def get_peft_state_maybe_zero_3(named_params, bias):
    """Collect LoRA adapter weights, accounting for ZeRO stage 3.

    Returns:
        dict[str, torch.Tensor]: State dict of LoRA (and optionally bias)
        parameters.
    """
    if bias == "none":
...
def get_peft_state_non_lora_maybe_zero_3(named_params, require_grad_only=True):
    """Collect non-LoRA parameters that still require gradients.

    Returns:
        dict[str, torch.Tensor]: Parameter name to CPU tensor mapping.
    """
...
def get_mm_adapter_state_maybe_zero_3(named_params, keys_to_match):
    """Collect multimodal adapter parameters for checkpointing.

    Returns:
        dict[str, torch.Tensor]: CPU tensors keyed by parameter name.
    """
...
def find_all_linear_names(model):
    """Return a list of ``torch.nn.Linear`` module names for LoRA injection.

    Returns:
        list[str]: Candidate module names.
    """
    cls = torch.nn.Linear
...
def safe_save_model_for_hf_trainer(trainer: transformers.Trainer,
    ...
    Returns:
        None: Side-effect only helper invoked during checkpointing.
    """
...
def smart_tokenizer_and_embedding_resize(
    ...
    Returns:
        None: The tokenizer/model objects are mutated in-place.
    """
...
def _tokenize_fn(strings: Sequence[str],
    ...
    Returns:
        Dict[str, list[torch.Tensor]]: Token ids and lengths aligned with the
        tokenizer's padding strategy.

    This helper is used throughout the data module to convert the conversational
    transcripts into tensors. The lengths it returns are later used by
    ``_mask_targets`` to determine which tokens should contribute to the loss.
    """
    tokenized_list = [
        tokenizer(
            text,
...
def _mask_targets(target, tokenized_lens, speakers):
    """Mask non-loss tokens in the autoregressive target tensor.

    Returns:
        None: ``target`` is modified in-place to set ignored tokens to
        ``IGNORE_INDEX``.

    The masking logic ensures that only assistant tokens contribute to the
    loss. When generating multiple masks per response you'll likely want to
    re-use this logic while carefully constructing the conversation so that
    each ``[MASK-DECODE]`` region lines up with a contiguous assistant span.
    """
    # cur_idx = 0
...
def _add_speaker_and_signal(header, source, get_conversation=True):
    ...
    Returns:
        str: The flattened conversation string when ``get_conversation`` is
        ``True``; otherwise the header is returned unchanged.

    The function injects role prefixes such as ``### Human:`` to conform with
    the LLaVA conversation template. Keeping track of these markers is crucial
    when designing prompts that request multiple masks within a single
    response—each mask should sit inside an assistant turn to benefit from the
    same masking behaviour described above.
    """
    BEGIN_SIGNAL = "### "
    END_SIGNAL = "\n"
    conversation = header
...
def preprocess_multimodal(
    ...
    """Inject image tokens and optional start/end markers into prompts.

    Returns:
        Sequence[dict]: Conversation dictionaries with normalized image tokens.

    The ``[MASK-ENCODE]`` / ``[MASK-DECODE]`` tokens that enable feature
    passing depend on this preprocessing step to appear in the right order.
    When authoring prompts for multiple masks in one response, ensure that each
    mask request retains the expected image token layout so that downstream
    processors can identify where to slot in the visual embeddings.
    """
...
def preprocess_llama_2(
    ...
    """Convert conversation dictionaries into LLaMA-2 style prompts.

    Returns:
        Dict[str, torch.Tensor]: Token IDs and masked targets for each
        conversation.

    This function is where multiple rounds (and thus multiple mask requests)
    get linearized into a single sequence. When adapting to intra-response
    multi-mask decoding, ensure that the generated sequence still alternates
    correctly between human and assistant turns so that the training objective
...
def preprocess_v1(
    ...
    """Legacy prompt builder that mirrors the original LLaVA template.

    Returns:
        Dict[str, torch.Tensor]: Token IDs and masked targets ready for
        training.

    ``preprocess_v1`` and the other ``preprocess_*`` variants differ mainly in
    how they insert separators. When experimenting with multi-mask prompts you
    may want to ensure the chosen template still places ``[MASK-DECODE]`` tokens
    inside assistant responses so the loss masking remains valid.
    """
    conv = conversation_lib.default_conversation.copy()
    roles = {"human": conv.roles[0], "gpt": conv.roles[1]}
...
def preprocess_mpt(
    ...
    """Prompt builder for MosaicML MPT-style separator patterns.

    Returns:
        Dict[str, torch.Tensor]: Token IDs and masked targets following the MPT
...
def preprocess_plain(
    ...
    """Simplified preprocessing that assumes a single human/assistant turn.

    Returns:
        Dict[str, list[torch.Tensor]]: Token IDs and masked targets.
    """
...
def preprocess_plain_gen(
    ...
    """Plain preprocessing with support for generation tokens.

    Returns:
        Dict[str, torch.Tensor]: Token IDs and masked targets for training.
    """
    # add end signal and concatenate together
    #conv = conversation_lib.default_conversation.copy()
...
def preprocess(
    ...
    """Primary preprocessing entry point used by both training and inference.

    Returns:
        Dict[str, torch.Tensor]: Token IDs, labels, and auxiliary metadata.
    ...
    The ``info`` dictionary can request additional behaviours such as enabling
    generation tokens. When experimenting with multiple masks per response you
    will likely route through this function so that the resulting dataset still
    contains the ``extra_replacement`` metadata consumed during inference.
    """
    # hack
...
def find_brackets(x):
    """Locate bracketed placeholders such as ``[MASK-DECODE:...]``.

    Returns:
        list[str]: All recognized bracket tokens present in ``x``.
    """
...
def remove_prefix(x):
    """Identity helper kept for backwards compatibility.

    Returns:
        str: Unmodified input string ``x``.
    """
...
def get_replacement_len(s):
    """Return the number of tokens that should replace a placeholder.

    Returns:
        int: Token length for the placeholder ``s``.

    ``IMAGE256`` placeholders expand to 256 tokens (one per 16x16 patch), while
    the encode/decode tokens expand to a single embedding. This logic becomes
    important when inserting multiple ``[MASK-DECODE]`` tokens in a single
    prompt because it determines how many tokens the tokenizer should reserve
...
class LazySupervisedDataset(Dataset):
    """Dataset for supervised fine-tuning with additional commentary.

    The executable behaviour mirrors ``train.py`` but the descriptive text
    emphasizes how conversation placeholders are expanded into multimodal
    tensors. Those expansions are the key levers for enabling multiple
    sequential mask predictions within a single assistant response.
    """
    ...
    def get_tensors_from_str(self,x):
        """Lookup cached embeddings associated with ``[PROMPT]`` tokens.

        Returns:
            torch.Tensor: A ``(1, 1024)`` tensor representing the stored
            feature. Unknown prompts yield a zero vector so downstream code can
            continue gracefully.
        """
        x = x.replace('[','',).replace(']','',)
    ...
    def get_dataset_indices(self):
        """Return the index boundaries for each dataset chunk.

        Returns:
            list[list[int]]: List of index ranges mirroring the dataset order.
        """
        return list([
    ...
    def get_dataset_weight(self):
        """Placeholder weights for data sampling.

        Returns:
            list[int]: Equal weights. Override to rebalance datasets.
        """
    ...
    def get_bitmask_decode(self, image_file_lst):
        """Fetch decode supervision for ``[MASK-DECODE]`` placeholders.

        Returns:
            tuple[Dict, Optional[int]]: Processor-ready mask tensors and the
    ...
    def build_query(self,x):
        """Dispatch serialized placeholders to modality-specific loaders.
    ...
    @property
    def lengths(self):
        """Approximate token length for curriculum/packing heuristics."""
        length_list = []
    ...
    def modality_lengths(self):
        """Signed lengths indicating the presence/absence of images."""
        length_list = []
        for sample in self.list_data_dict:
    ...
    def dataset_index(self,i):
        """Map a flat index into the dataset partition it originated from."""
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        """Assemble a fully materialized training example.

        Returns:
            Dict[str, Any]: Token IDs, labels, and multimodal side inputs.
        """
...
def gather_by_key(data):
    """Collate heterogeneous multimodal batches produced by the dataset.

    Returns:
        dict[str, Any]: A mapping from modality name to batched tensors plus an
        ``info`` list describing the source of each row.

    This helper is central to feature passing: mask/bbox embeddings are stored
    under the ``extra_replacement`` modality and later expanded into the
    placeholders consumed by the model.
    """
    gathered_inputs = {}
    info = []
...
def clean(x):
    """Normalize strings before embedding lookup.

    Returns:
        str: Lower-cased and punctuation-stripped string.
    """
    x = x.lower().strip()
    x = x.replace('.','')
    return x
...
class DataCollatorForSupervisedDataset(object):
    """Collate examples for supervised fine-tuning with feature passing notes.

    The collator owns the final steps of weaving multimodal tensors together,
    so we explicitly point out where ``extra_replacement`` is assembled. When
    multiple masks are predicted in a single response those tensors are the
    glue that ties each ``MASK-DECODE`` token back to the encoded context from
    previous rounds in the same turn.
    """

    tokenizer: transformers.PreTrainedTokenizer

    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        """Batch and pad a list of dataset instances.

        Returns:
            Dict[str, Any]: Batched tensors including ``extra_replacement``
            metadata for feature passing.
        """
        ...
        # ``extra_replacement`` contains the encoded image/mask/bbox features
        # that will be re-injected into the token sequence whenever a
        # ``MASK-DECODE`` placeholder is encountered. When generating multiple
        # masks per response this tensor effectively becomes a feature queue
        # that later decodes pop from using ``mask_encode_ref`` indices.
...
def make_supervised_data_module(tokenizer: transformers.PreTrainedTokenizer,
    ...
    Returns:
        Dict[str, Any]: The datasets/collator required by ``LLaVATrainer``.

    During inference this function constructs a ``LazySupervisedDataset`` that
    uses ``inference_conv`` as the sole conversation source. This is how the
    CLI feeds updated conversations (with feature passing placeholders) into
    the model.
    """
...
def train():
    """CLI entry point for launching SegLLM training.

    Returns:
        None: Executes the training or evaluation routine based on CLI args.
    """
    global local_rank
    pipe = None
