...
import logging
import pathlib
from typing import Dict, Optional, Sequence, List, TypedDict
import cv2
import torch
...
from explanatory_seg_dataset import ExplanatorySegDataset
...
class DataArguments:
    ...
    dataset_folder: str = '/shared/nas2/blume5/sp25/partonomy/partonomy_private/data/partonomy_descriptors'
    partonomy_dataset_split: str = 'partimagenet|pascal_part|paco_lvis'
    partonomy_train_dataset_path: str = '/shared/nas2/blume5/sp25/partonomy/partonomy_private/data/partonomy_descriptors/{dataset_split}/{dataset_split}_qa_pairs_train.json'
    partonomy_val_dataset_path: str = '/shared/nas2/blume5/sp25/partonomy/partonomy_private/data/partonomy_descriptors/{dataset_split}/{dataset_split}_qa_pairs_val.json'
...
def _add_speaker_and_signal(header, source: list[dict], get_conversation=True):
...
class CollatedBatch(TypedDict):
    ...
    generation_target: dict
    extra_replacement: dict
...
class DataCollatorForSupervisedDataset(object):
    ...
    tokenizer: transformers.PreTrainedTokenizer
    unpack_multiple_choice: bool = False

    def __call__(self, instances: list[Sequence[Dict]]) -> CollatedBatch:
        if self.unpack_multiple_choice:
            assert len(instances) == 1, 'Batch size should be 1 so multiple answer choices can be unpacked for forward pass'
            instances = instances[0] # [[dict, dict, ...]] --> [dict]
            assert len(instances) > 1, 'Each '
        else:
            # list[[dict], [dict], ...] -> list[dict, dict, ...]
            new_instances = []
            for instance_list in instances:
                assert len(instance_list) == 1, 'Each instance list should contain only one conversation (the correct answer)'
                new_instances.append(instance_list[0])
        ...
        batch['conversations'] = [x['conversation'] for x in instances]
        batch['image_paths'] = [x['image_path'] for x in instances]

        # TODO: collection for batch['mask']
...
class SupervisedDataModule(TypedDict):
    train_dataset: ExplanatorySegLazySupervisedDataset
    eval_dataset: Optional[ExplanatorySegLazySupervisedDataset]
...
def make_supervised_data_module(tokenizer: transformers.PreTrainedTokenizer,
                                ...
                                inference_conv: Optional[Dict] = None) -> SupervisedDataModule:
    ...
    if is_eval:
        eval_dataset = ExplanatorySegLazySupervisedDataset(
            tokenizer=tokenizer,
            data_args=data_args,
            is_eval=True
        )
    ...
    # Train
    print(">> (segllm:train_eseg) loading ExplanatorySegDataset...")
    ...
    dataset_splits = data_args.partonomy_dataset_split.split("|")  # 'partimagenet|pascal_part|paco_lvis|partonomy'
    train_paths = []
    for dataset_split in dataset_splits:
        if dataset_split == 'partonomy':
            continue
        else:
            train_paths.append(data_args.partonomy_train_dataset_path.format(dataset_split=dataset_split))

    train_explanatory_seg_datasets = []
    for question_type in train_question_types:
        print(">> (segllm:train_eseg) train_question_types: ", question_type)
        train_explanatory_seg_datasets.extend(
            [
    ...
    eval_question_types = [  # NOTE: We only train on with label case
        QuestionType.IDENTIFICATION_WITH_LABEL,
        # QuestionType.POSITIVE_WITH_LABEL,
        # QuestionType.NEGATIVE_WITH_LABEL,
        # QuestionType.DIFFERENCE,  # NOTE: We don't train on difference, part2whole, whole2part
    ...
    dataset_splits = [data_args.partonomy_dataset_split.split("|")[0]]  # 'partimagenet|pascal_part|paco_lvis|partonomy'
    eval_paths = []
    for dataset_split in dataset_splits:
        if dataset_split == 'partonomy':
            continue
        else:
            eval_paths.append(data_args.partonomy_val_dataset_path.format(dataset_split=dataset_split))

    eval_explanatory_seg_datasets = []
    for question_type in eval_question_types:
        print(">> (segllm:train_eseg) eval_question_types: ", question_type)
        eval_explanatory_seg_datasets.extend(
            [
                ExplanatorySegDataset(
                    path,
                    question_type=question_type,
    ...
    for ranks in ranks_LST:
        ...
        if local_rank in ranks:
            ...
            if model_args.vision_tower is not None:
                ...
                for para in vision_tower.parameters():
            ...
            # NOTE: Enforce contiguous
            def enforce_contiguous(module: torch.nn.Module, fix: bool = True):
                for name, p in module.named_parameters(recurse=True):
                    if not p.is_contiguous():
                        print(f"[non-contiguous param] {name} {tuple(p.shape)}")
                        if fix:
                            p.data = p.data.contiguous()
                for name, b in module.named_buffers(recurse=True):
                    if not b.is_contiguous():
                        print(f"[non-contiguous buffer] {name} {tuple(b.shape)}")
                        if fix:
                            # Re-register contiguous view for buffers
                            mod = module
                            parts = name.split(".")
                            for part in parts[:-1]:
                                mod = getattr(mod, part)
                            mod._buffers[parts[-1]] = b.contiguous()

            # After building/adjusting the model and before Trainer/optimizer:
            enforce_contiguous(model, fix=True)

            model.to(torch.bfloat16)  # XXX
