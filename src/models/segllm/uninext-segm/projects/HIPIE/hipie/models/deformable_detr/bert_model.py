...
class BertEncoder(nn.Module):
    def __init__(self, cfg):
        ...
        if self.bert_name == "bert-base-uncased":
            # config = BertConfig.from_pretrained("./uninext-segm/projects/HIPIE/%s"%self.bert_name)
            bert_path = str(Path(__file__).parent.parent.parent.parent / 'bert-base-uncased') # ANSEL
            config = BertConfig.from_pretrained(bert_path) # ANSEL
            config.gradient_checkpointing = self.cfg.MODEL.LANGUAGE_BACKBONE.USE_CHECKPOINT
            # self.model = BertModel.from_pretrained("./uninext-segm/projects/HIPIE/%s" % self.bert_name, add_pooling_layer=False, config=config)
            self.model = BertModel.from_pretrained(bert_path, add_pooling_layer=False, config=config)
